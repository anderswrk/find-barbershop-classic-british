/**
 * Kliv Auth SDK - Simple authentication and user management for Kliv apps
 *
 * Key behaviors:
 * - Singleton pattern - only one instance exists
 * - Auth tokens stored in httpOnly cookies (not accessible via JS)
 * - Cookies expire after 7 days
 * - No token refresh needed - just use until expiry
 * - All methods are async and return the user object when applicable
 * - Password reset sends email with reset link (actual reset handled via web form)
 *
 * User management:
 * - List, get, update, and delete users
 * - Manage user group membership
 * - List available groups with descriptions
 */
class KlivAuth {
    constructor() {
    ...
    async requestPasswordReset(email) {
        const response = await fetch('/api/v1/user/reset_password', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
